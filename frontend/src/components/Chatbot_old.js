...
const Chatbot = () => {
  ...
  // Enhanced formatting function with Claude AI-like styling
  const formatResponseContent = (content) => {
    const paragraphs = content.split("\n\n").filter((p) => p.trim());

    return (
      <Box sx={{ mt: 1, mb: 1, display: "flex", flexDirection: "column", gap: 2 }}>
        {paragraphs.map((paragraph, pIdx) => {
          ...
          // Handle bold text and code formatting
          const renderParagraphWithFormatting = (text) => {
            const parts = [];
            let lastIndex = 0;

            // Match **text** for bold, `code` for inline code
            const regex = /\*\*(.+?)\*\*|`(.+?)`/g;
            let match;
            ...
            while ((match = regex.exec(text)) !== null) {
              ...
              if (match[1]) {
                // Bold text
                parts.push(
                  <span key={`bold-${parts.length}`} style={{ fontWeight: 700, color: "#1f2937" }}>
                    {match[1]}
                  </span>
                );
              } else if (match[2]) {
          ...
          return (
            ...
            >
              {renderParagraphWithFormatting(paragraph)}
            </Typography>
          );
        })}
      </Box>
